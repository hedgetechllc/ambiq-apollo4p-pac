...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn cmdcmp(&mut self) -> CmdcmpW<IntsetSpec> {
        CmdcmpW::new(self, 0)
    }
    #[doc = "Bit 1 - Transmit FIFO empty."]
    #[inline(always)]
    #[must_use]
    pub fn txe(&mut self) -> TxeW<IntsetSpec> {
        TxeW::new(self, 1)
    }
    #[doc = "Bit 2 - Transmit FIFO Overflow (only occurs when SW writes to a full FIFO)."]
    #[inline(always)]
    #[must_use]
    pub fn txo(&mut self) -> TxoW<IntsetSpec> {
        TxoW::new(self, 2)
    }
    #[doc = "Bit 3 - Receive FIFO underflow (only occurs when SW reads from an empty FIFO)"]
    #[inline(always)]
    #[must_use]
    pub fn rxu(&mut self) -> RxuW<IntsetSpec> {
        RxuW::new(self, 3)
    }
    #[doc = "Bit 4 - Receive FIFO overflow (cannot happen in MSPI design -- MSPI bus pins will stall)"]
    #[inline(always)]
    #[must_use]
    pub fn rxo(&mut self) -> RxoW<IntsetSpec> {
        RxoW::new(self, 4)
    }
    #[doc = "Bit 5 - Receive FIFO full"]
    #[inline(always)]
    #[must_use]
    pub fn rxf(&mut self) -> RxfW<IntsetSpec> {
        RxfW::new(self, 5)
    }
    #[doc = "Bit 6 - DMA Complete Interrupt"]
    #[inline(always)]
    ...
    #[inline(always)]
    #[must_use]
    pub fn cqcmp(&mut self) -> CqcmpW<IntsetSpec> {
        CqcmpW::new(self, 8)
    }
    #[doc = "Bit 9 - Command Queue Update Interrupt. Issued whenever the CQ performs an operation where address bit\\[0\\]
is set. Useful for triggering CURIDX interrupts."]
    #[inline(always)]
    #[must_use]
    pub fn cqupd(&mut self) -> CqupdW<IntsetSpec> {
        CqupdW::new(self, 9)
    }
    #[doc = "Bit 10 - Command Queue is Paused."]
    #[inline(always)]
    #[must_use]
    pub fn cqpaused(&mut self) -> CqpausedW<IntsetSpec> {
        CqpausedW::new(self, 10)
    }
    #[doc = "Bit 11 - Command Queue Error Interrupt"]
    #[inline(always)]
    #[must_use]
    pub fn cqerr(&mut self) -> CqerrW<IntsetSpec> {
        CqerrW::new(self, 11)
    }
    #[doc = "Bit 12 - Scrambling Alignment Error. Scrambling operations must be aligned to word (4-byte) start address."]
    #[inline(always)]
    #[must_use]
    pub fn screrr(&mut self) -> ScrerrW<IntsetSpec> {
        ScrerrW::new(self, 12)
    }
    #[doc = "Bit 13 - MSPI is dma target as well as dma source, which may result in deadlock."]
    #[inline(always)]
    #[must_use]
    pub fn apbdmaerr(&mut self) -> ApbdmaerrW<IntsetSpec> {
