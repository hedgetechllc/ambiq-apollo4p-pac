...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn txe(&mut self) -> TxeW<IntenSpec> {
        TxeW::new(self, 1)
    }
    #[doc = "Bit 2 - Transmit FIFO Overflow (only occurs when SW writes to a full FIFO)."]
    #[inline(always)]
    #[must_use]
    pub fn txo(&mut self) -> TxoW<IntenSpec> {
        TxoW::new(self, 2)
    }
    #[doc = "Bit 3 - Receive FIFO underflow (only occurs when SW reads from an empty FIFO)"]
    #[inline(always)]
    #[must_use]
    pub fn rxu(&mut self) -> RxuW<IntenSpec> {
        RxuW::new(self, 3)
    }
    #[doc = "Bit 4 - Receive FIFO overflow (cannot happen in MSPI design -- MSPI bus pins will stall)"]
    #[inline(always)]
    #[must_use]
    pub fn rxo(&mut self) -> RxoW<IntenSpec> {
        RxoW::new(self, 4)
    }
    #[doc = "Bit 5 - Receive FIFO full"]
    #[inline(always)]
    #[must_use]
    pub fn rxf(&mut self) -> RxfW<IntenSpec> {
        RxfW::new(self, 5)
    }
    #[doc = "Bit 6 - DMA Complete Interrupt"]
    #[inline(always)]
    ...
    #[inline(always)]
    #[must_use]
    pub fn cqcmp(&mut self) -> CqcmpW<IntenSpec> {
        CqcmpW::new(self, 8)
    }
    #[doc = "Bit 9 - Command Queue Update Interrupt. Issued whenever the CQ performs an operation where address bit\\[0\\]
is set. Useful for triggering CURIDX interrupts."]
    ...
    #[inline(always)]
    #[must_use]
    pub fn screrr(&mut self) -> ScrerrW<IntenSpec> {
        ScrerrW::new(self, 12)
    }
    #[doc = "Bit 13 - MSPI is dma target as well as dma source, which may result in deadlock."]
    #[inline(always)]
    #[must_use]
    pub fn apbdmaerr(&mut self) -> ApbdmaerrW<IntenSpec> {
